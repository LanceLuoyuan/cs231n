...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    pass
    first_full_scores = X.dot(W1) + b1
    relu_scores = np.maximum(first_full_scores,np.zeros((N,b1.shape[0])))
    ...
    loss = np.sum(-np.log(scores[xrange(N),y]))
    ...
    dscores[range(N),y] -= 1
    dscores /= N
    dW2 = np.dot(relu_scores.T,dscores)
